...
- Access is gated by `AuditEntitlementService`: a free-run quota (`config('audit.free_reports_limit')` + per-user `audit_bonus_free_runs` bonus) and a per-tier subscription allowance read from the plan's product metadata, metered per calendar month. One key per tier, no aliases — `audit_diagnostic_credits`, `audit_deep_ai_credits`, `audit_expert_credits`. Diagnostic falls back to the lifetime free-run quota only when the plan grants it no allowance. The catalog has three tiers (Diagnostic $49, Deep AI $119, Expert $999); `automated` was retired on 2026-08-24.
