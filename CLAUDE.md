...
SaaSykit is the *boilerplate*; the actual product is a **code-audit service** layered on top — the backend counterpart to the frontend's "vibecode rescue" landing. A visitor submits a `repo_url` (`AuditRequest`, UUID-keyed, owned by a Tenant via `scopeForTenant`; claimable by email before signup); a pipeline clones and analyzes the repo and emails back a gated report. Almost all of this lives in `app/Services/AuditReport/`, orchestrated by `AuditPipeline::run()`, whose stages are:
...
Failures throw `AuditNotAnalyzableException` → `AuditRequestService::closeNotAnalyzable()`: the request goes to the terminal `not_analyzable` status, its run is refunded (`AuditEntitlementService::refund()`, stamped `credit_refunded_at`), and the customer is emailed to grant access and run a **new** audit — nothing restarts the old one. Dashboard launches run `RepositoryCloner::preflight()` before charging. Note `AuditDeltaService` / `AuditBenchmarkService` are **not** part of this run — they're applied at report *view* time in `AuditReportController`.
